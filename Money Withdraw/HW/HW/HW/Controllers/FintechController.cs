...
namespace FintechRestAPI.Controllers
{
    ...
    public class FintechController : ControllerBase
    {
        ...
        // This API endpoint allows a user to withdraw a certain amount of money from their Fintech account by specifying the account ID and the amount to withdraw
        // In case of a bad request (e.g. missing parameters), return a HTTP 400 error response
        [ProducesResponseType(400)]
        // In case of success, return a HTTP 200 OK response
        [ProducesResponseType(200)]
        // If the specified account ID does not exist, return a HTTP 404 Not Found response
        [ProducesResponseType(404)]
        public IActionResult WithdrawAmount(int id, double amount)
        {
            // Get the Fintech account object with the specified ID from the data store
            Fintech account = _fintech.GetItem(id);
            // If the account object is null, the specified account ID does not exist, so return a HTTP 404 Not Found response
            if (account == null)
            {
                return NotFound();
            }

            // Attempt to withdraw the specified amount from the Fintech account with the specified ID
            bool result = _fintech.WithdrawAmount(id, amount);
            // If the withdrawal was successful, return a HTTP 200 OK response with a success message
            // If the withdrawal was not successful (e.g. insufficient funds), return a HTTP 400 Bad Request response with an error message
